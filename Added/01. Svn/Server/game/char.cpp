...
void CHARACTER::Restart(BYTE bSubCMD)
{
	...
	if (bSubCMD == SCMD_RESTART_TOWN)
	{
		if (IsHack())
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, LC_STRING("???? ????? ?? ?? ???????. (%d?? ????)", iTimeToDead - (180 - g_nPortalLimitTime)));
				return;
			}
		}
		...
		if (iTimeToDead > 173)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_STRING("???? ???????? ????? ?? ?? ???????. (%d ?? ????)", iTimeToDead - 173));
			return;
		}
	}
	//END_PREVENT_HACK
...
void CHARACTER::SetPetWindowType(uint8_t pet_window_type)
{
	...
	if (pet_window_type == 1)
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_STRING("You can open it 3 seconds after closing the pet stats transformation window."));
			return;
		}
